...
export const AuthPage: React.FC = () => {
  ...
  const handleQuickLogin = async () => {
    ...
    setLoading(true);
    try {
      await signInWithTestUser("mudassirjee1000@gmail.com", "Mudassir");
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to sign in. Please try email login.");
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 text-slate-900 font-sans">
      ...
      <div className="md:w-7/12 p-6 sm:p-12 md:p-16 flex items-center justify-center">
        <div className="w-full max-w-md">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-2.5">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-amber-800 leading-relaxed">{oauthError}</p>
                    {oauthError.toLowerCase().includes("access_denied") || oauthError.includes("403") ? (
                      <div className="mt-2 text-2xs text-amber-800 bg-amber-100/70 p-2.5 rounded-lg leading-normal space-y-1.5">
                        <p className="font-semibold text-amber-900">Why Google returned 403 Forbidden:</p>
                        <p>1. <strong>Preview iframe:</strong> Google strictly blocks rendering login inside iframes. The button below now opens Google in a full window.</p>
                        <p>2. <strong>Google Cloud Testing Mode:</strong> In Google Cloud Console &rarr; <em>APIs &amp; Services</em> &rarr; <em>OAuth consent screen</em>, verify your app is <strong>External</strong> and add <code className="bg-amber-200/80 px-1 py-0.5 rounded text-amber-900">mudassirjee1000@gmail.com</code> under <strong>Test users</strong>.</p>
                        <p className="pt-1 text-blue-900 font-medium">&rarr; Or simply click <strong>"Instant 1-Click Access"</strong> below to enter and use the store immediately!</p>
                      </div>
                    ) : null}
                  </div>
                </div>
                <button
                  ...
                  className="text-amber-500 hover:text-amber-800 p-1 text-xs cursor-pointer font-bold"
          ...
          {/* Option A: Quick Login & Google Login Buttons */}
          {mode !== "forgot" && (
            <div className="mb-6 space-y-3">
              {/* Instant 1-Click Access for Mudassir */}
              <button
                type="button"
                id="btn-quick-login"
                onClick={handleQuickLogin}
                disabled={loading}
                className="w-full flex items-center justify-between px-4 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl text-xs font-semibold shadow-md shadow-blue-500/20 transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 disabled:opacity-60 cursor-pointer text-left"
              >
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    <div className="font-bold text-white text-xs">Instant 1-Click Sign In</div>
                    <div className="text-2xs text-blue-100 font-normal">Enter as mudassirjee1000@gmail.com</div>
                  </div>
                </div>
                <span className="text-2xs bg-white/20 px-2.5 py-1 rounded-lg font-medium text-white shrink-0">
                  Open Store &rarr;
                </span>
              </button>
